			...
			<scroll-view scroll-y="true">
					...
					<view class="search-result">
						<view class="search-result-item" v-for="(item,index) in searchList" :key="index" @tap="handleToDetail(item.id)">
							<view class="search-result-word">
								<view>{{item.name}}</view>		
								<view><image src="../../static/dujia.png" mode=""></image>{{item.artists[0].name}} - {{item.album.name}}</view>
							</view>
				...
				<block v-else-if="searchType==3">
					<view class="search-suggest">
						...
						<view class="search-suggest-item" v-for="(item,index) in searchSuggest" :key="index" @tap="handleToWord(item.ketword)">
	...
	export default {
		...
		methods: {
			handleToWord(word){
				...
				this.handleToSearch(handleToWord)
			},
			handleToSearch(word){
